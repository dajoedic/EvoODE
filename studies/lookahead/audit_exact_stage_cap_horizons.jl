...
const OLD_CONFIG_FINGERPRINT = "0825cdc88d9264a0"
const OLD_PHASE_B_FINGERPRINT = "ca02ea284d621f6d"
...
function write_report(path::String, rows)
    ...
    elseif selected == AUDIT_CAP_POLICY.lookahead_horizon
        push!(
            lines,
            "",
            "Part 3 was executed: the default look-ahead horizon is now `$(AUDIT_CAP_POLICY.lookahead_horizon)` in `LookAheadStageCapPolicy`, `LOOKAHEAD_CAP_POLICY`, and `LOOKAHEAD_CAP_POLICY_REGRESSION`.",
            "",
            "## Fingerprints",
            "",
            markdown_table(
