...
function _wp_n1_completed(path::String, fingerprint::String)
...
function _wp_n1_write_summary(history_path::String, summary_path::String, fingerprint::String)
    records = Any[]
...
function main()
    ...
    allow_dim2 = lowercase(strip(get(ENV, "WP_N1_ALLOW_DIM2", ""))) in ("1", "true", "yes")
    if dim != 1 && !allow_dim2
    ...
    completed = fresh_requested() ? Set{Tuple{String, Int, Int, Int}}() : _wp_n1_completed(history_path, fingerprint)
    provenance = (git_hash = "not_collected", git_dirty = nothing)
    total = length(variants) * length(base_systems) * length(WP_N1_IC_SETS) * length(WP_N1_SEEDS)
    run_index = 0
    appended = 0
    skipped = 0
    ...
    _wp_n1_write_summary(history_path, summary_path, fingerprint)
